        ...
        <div class="profile-card shadow-sm">
            <c:choose>
                <c:when test="${not empty user.u_image}">
                    <img src="${pageContext.request.contextPath}/images/profilepics/${user.u_image}" class="profile-img" alt="Profile Picture">
                </c:when>
                <c:otherwise>
            ...
            <h2>
                <i class="bi bi-person-circle me-2"></i> ${user.u_firstname} ${user.u_lastname}
            </h2>
            <form action="${pageContext.request.contextPath}/ProPicUpdateServlet" method="POST" enctype="multipart/form-data" class="needs-validation" novalidate>
                <div class="mb-3">
                    <label class="form-label" for="profilepic"><i class="bi bi-camera me-2"></i>Change Photo</label>
                    <input type="file" id="profilepic" name="profilepic" class="form-control" accept="image/*" required>
    ...
    <div class="modal-content bg-white text-dark">
      <form action="${pageContext.request.contextPath}/deleteUserServlet" method="POST">
        ...
        <div class="modal-body">
          <p>
            Are you sure you want to <span class="fw-bold text-danger">delete your account</span>?<br>
            This action is <span class="fw-bold">permanent</span> and cannot be undone.<br>
            Please type your User ID (<strong>${userid}</strong>) to confirm.
          </p>
