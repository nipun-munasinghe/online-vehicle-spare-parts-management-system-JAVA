...
public class ManagerService {
	...
	public boolean addManager(managerModel manager) throws SQLException, ClassNotFoundException {
		String sql = "INSERT INTO manager (m_firstname, m_lastname, m_email, phone, m_password, M_status) "
				+ "VALUES (?, ?, ?, ?, ?, ?)";
	...
	public List<managerModel> getAllManagers() throws SQLException, ClassNotFoundException {
		...
		String sql = "SELECT m_firstname, m_lastname, m_email, phone, M_status FROM manager";
		...
		try (Connection conn = DBConnection.getConnection();
			...
			while (rs.next()) {
				managers.add(new managerModel(rs.getString("m_firstname"), rs.getString("m_lastname"),
						rs.getString("m_email"), rs.getString("phone"), "", // Explicitly exclude password
	...
	// Update manager status by email
